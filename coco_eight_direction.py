...
#########################################################################################
        ...
        custom_angle = [45,90,135,180,-135,-90,-45]                    
        self.new_custom_angle = []
        
        for i in custom_angle:

            i = i*np.pi/180
            self.new_custom_angle.append(i)

        print('客製角度list:',self.new_custom_angle)
...
####################################### affine transformation  ###################################################
        ...
        if (idx % 2) != 0:                                        
            angle_length = len(self.new_custom_angle)
            custom_angle_idx = random.randint(1,angle_length-1)
            angle_10 = float(torch.rand(1)) * 0.4 - 0.2
            angle = self.new_custom_angle[custom_angle_idx] + angle_10
            shiftx = 0
            shifty = 0
            scale = 1
...
###################################################################################################################
        ...
        cmap = trt_pose.plugins.generate_cmap(counts, peaks, self.target_shape[0], self.target_shape[1], stdev, int(stdev * 5))

        paf = trt_pose.plugins.generate_paf(self.connections[idx][None, ...], self.topology,counts, peaks,self.target_shape[0], self.target_shape[1], stdev)
